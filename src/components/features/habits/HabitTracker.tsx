...
import { Check, Plus, Calendar, TrendingUp, Target, BookOpen, Headphones, Loader2, ArrowLeft, Trash2, AlertTriangle } from 'lucide-react';
import { collection, query, where, getDocs, doc, updateDoc, getDoc, addDoc, deleteDoc, Timestamp } from 'firebase/firestore';
...
export function HabitTracker({ user }: HabitTrackerProps) {
  ...
  const updateLocalHabitState = (habitId: string) => {
    ...
    if (newTodayCompleted) {
      ...
      if (habitCard) {
        ...
        setTimeout(() => {
          habitCard.classList.remove('scale-[1.02]', 'bg-green-50', 'shadow-lg');
        }, 800);
      }
    }
  };
  ...
  if (currentView === 'detail' && selectedHabit) {
    return (
      <div className="h-full overflow-y-auto p-6 pb-24 relative">
          ...
          <CardContent>
              ...
              {selectedHabit.weeklyProgress.map((completed, index) => (
                  ...
                  >
                      ...
                      <span className="text-xs font-medium text-gray-500">{getDayLabel(index)}</span>
                </div>
              ))}
            </div>
          </CardContent>
          ...
          {habits.length > 0 ? (
            <>
              ...
              <div className="space-y-3">
                <h3 className="text-sm font-medium text-gray-700">Đánh dấu nhanh:</h3>
                <div className="space-y-2">
                  {habits.map(habit => (
                    ...
                    >
                      <div className="flex items-center" onClick={() => viewHabitDetail(habit)} style={{cursor: 'pointer'}}>
                        <div className={`w-8 h-8 rounded-lg ${habit.bgColor} flex items-center justify-center mr-3`}>
                          <habit.icon className={`h-4 w-4 ${habit.textColor}`} />
                        </div>
                        <span className={`text-sm font-medium ${habit.todayCompleted ? 'text-green-700' : 'text-gray-700'}`}>
                          {habit.title}
                        </span>
                      </div>
                      <Button
            ...
            <div className="flex flex-col items-center justify-center py-4 text-center">
              ...
              <p className="text-gray-600 mb-4">Thêm thói quen mới để bắt đầu theo dõi tiến trình học tập của bạn</p>
              <Button 
                onClick={createDefaultHabits}
        ...
        <Card className="mb-6 border-dashed border-2 border-gray-300 bg-gray-50">
          <CardContent className="p-8 text-center">
            <div className="flex flex-col items-center justify-center space-y-4">
              ...
              <div>
                <h3 className="text-lg font-medium text-gray-900">Chưa có thói quen nào</h3>
                <p className="text-gray-600 mt-1 mb-4">Thêm thói quen mới để bắt đầu theo dõi tiến trình học tập của bạn</p>
                <Button 
                  onClick={createDefaultHabits}
                  className="bg-blue-600 hover:bg-blue-700 text-white"
                >
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              Hành động này không thể hoàn tác và tất cả dữ liệu theo dõi sẽ bị mất.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
  ...
  const handleDeleteHabit = async () => {
    ...
    try {
      ...
      if (auth.currentUser && habitToDelete.id && !habitToDelete.id.startsWith('local-')) {
        try {
